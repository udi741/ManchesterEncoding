...
int main()
{
    ...
    standard = THOMAS;
    unsigned char data[] = {0xDE, 0xAD, 0xBE, 0xEF, 0x01, 0x90};
    ...
    {
        printf("\nEncoded data:\n");
        for (int i=0; i<ARRAY_LENGTH(encodedData); i++)
        ...
        {
            printf("\nDecoded data:\n");
            for (int i=0; i<ARRAY_LENGTH(decodedData); i++)
                printf("%02X ",decodedData[i]);
        }
        else if (status == ERROR_STANDARD)
            printf("Invalid conversion standard.\n");
        else if (status == INVALID_ENCODE)
            printf("Invalid encoded data.\n");
        else if (status == ERROR_SIZE)
            printf("Invalid array size. size must be multiple of 2\n");

    }
    else if(status == ERROR_STANDARD)
    {
        printf("Invalid conversion method.\n");
    }
